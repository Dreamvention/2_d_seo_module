...
class ControllerModuleDSEOModule extends Controller {
	...
	public function setting() {
		...
		// Action
		$data['href_setting'] = $this->url->link($this->route . '/setting', 'token=' . $this->session->data['token'], 'SSL');
		$data['href_custom_page'] = $this->url->link($this->route . '/custom_page', 'token=' . $this->session->data['token'], 'SSL');
		$data['href_export_import'] = $this->url->link($this->route . '/export_import', 'token=' . $this->session->data['token'], 'SSL');
		$data['href_instruction'] = $this->url->link($this->route . '/instruction', 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['install'] = $this->url->link($this->route . '/installModule', 'token=' . $this->session->data['token'], 'SSL');
		$data['uninstall'] = $this->url->link($this->route . '/uninstallModule', 'token=' . $this->session->data['token'], 'SSL');
		$data['store_setting'] = $this->url->link('setting/store', 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['htaccess'] = $this->{'model_module_' . $this->codename}->getFileData('htaccess');
		$data['robots'] = $this->{'model_module_' . $this->codename}->getFileData('robots');
	...
	public function custom_page() {
		...
		$custom_pages = $this->{'model_module_' . $this->codename}->getCustomPages();
	...
	public function export_import() {
		...
		$data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');
		$data['install'] = $this->url->link($this->route . '/installModule', 'token=' . $this->session->data['token'], 'SSL');
		$data['export'] = $this->url->link($this->route . '/export', 'token=' . $this->session->data['token'], 'SSL');
		$data['import'] = $this->url->link($this->route . '/import', 'token=' . $this->session->data['token'], 'SSL');
	...
	public function save() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if (isset($this->request->post['htaccess'])) {
				$this->{'model_module_' . $this->codename}->saveFileData('htaccess', $this->request->post['htaccess']);
			}
			if (isset($this->request->post['robots'])) {
				$this->{'model_module_' . $this->codename}->saveFileData('robots', $this->request->post['robots']);
			}
	...
	public function createDefaultCustomPage() {
		...
		if ($this->validate()) {
			$this->{'model_module_' . $this->codename}->createDefaultCustomPage();
	...
	public function addCustomPage() {
		...
		if (isset($this->request->post['custom_page']) && $this->validateAddCustomPage()) {
			$this->{'model_module_' . $this->codename}->addCustomPage($this->request->post['custom_page']);
	...
	public function editCustomPage() {
		...
		if (isset($this->request->post['route']) && isset($this->request->post['language_id']) && isset($this->request->post['target_keyword']) && $this->validateEditCustomPage()) {
			...
			$this->{'model_module_' . $this->codename}->editCustomPage($custom_page_data);
		}
	...
	public function deleteCustomPage() {
		...
		if (isset($this->request->post['selected']) && $this->validate()) {
			foreach ($this->request->post['selected'] as $route) {
				$this->{'model_module_' . $this->codename}->deleteCustomPage($route);
			}
			...
			if ((isset($this->request->files['upload'])) && (is_uploaded_file($this->request->files['upload']['tmp_name'])) && file_exists(DIR_SYSTEM . 'library/d_phpexcel.php')) {
				...
				if ($worksheet != null) {
					...
					foreach ($elements as $element) {
						...
						$this->{'model_module_' . $this->codename}->saveCustomPages($custom_pages);
					}
				}
			}
		}
	...
	public function menu_after($route, $data, &$output) {
		...
		if (file_exists(DIR_SYSTEM . 'library/d_simple_html_dom.php')) {
			...
			$menu_items = $this->{'model_module_' . $this->codename}->sortArrayByColumn($menu_items, 'sort_order');
	...
	public function dashboard_after($route, $data, &$output) {
		...
		if (file_exists(DIR_SYSTEM . 'library/d_simple_html_dom.php')) {
			...
			foreach ($seo_extensions as $seo_extension) {
				$info = $this->load->controller($this->codename . '/' . $seo_extension . '/dashboard');
				if ($info) $dashboards = array_merge($dashboards, $info);
			}
		
			$dashboards = $this->{'model_module_' . $this->codename}->sortArrayByColumn($dashboards, 'sort_order');
	...
	*/	
	public function getTargetKeywords() {
		...
		$data['target_keywords'] = $this->{'model_module_' . $this->codename}->getTargetKeywords($filter_data);
